...
fn successful_classifier_verdict_overrides_each_configured_default() {
    ...
    assert_eq!(claude.provider, BackendKind::Claude);
    ...
    assert_eq!(codex.provider, BackendKind::Codex);
...
fn capability_pins_force_claude_and_skip_weekly_routing() {
    ...
    assert_eq!(missing_connector.provider, BackendKind::Claude);
    ...
    assert_eq!(claude_signals.provider, BackendKind::Claude);
...
fn weekly_failover_pins_both_outputs_in_both_directions_by_default() {
    ...
    assert_eq!(to_claude.provider, BackendKind::Claude);
    ...
    assert_eq!(to_codex.provider, BackendKind::Codex);
...
fn both_providers_over_ceiling_keep_the_selected_provider() {
    ...
    for (verdict, provider) in [
        (Verdict::Codex, BackendKind::Codex),
        (Verdict::Claude, BackendKind::Claude),
    ] {
        let decision = decide(
...
fn explicit_provider_decisions_remain_outside_policy_routing() {
    ...
    let codex = decide_explicit(BackendKind::Codex, None, snapshot);
    assert_eq!(codex.provider, BackendKind::Codex);
    assert_eq!(codex.model, None);
    assert_eq!(codex.effort.as_deref(), Some(CODEX_EFFORT));
    ...
    let claude = decide_explicit(BackendKind::Claude, Some("sonnet".to_string()), snapshot);
    assert_eq!(claude.provider, BackendKind::Claude);
    ...
    let opencode = decide_explicit(BackendKind::Opencode, None, snapshot);
    assert_eq!(opencode.provider, BackendKind::Opencode);
