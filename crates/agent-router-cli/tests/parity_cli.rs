...
fn write_command_difference_with_secrets(project: &Path, claude_secret: &str, codex_secret: &str) {
    write_file(
        &project.join(".mcp.json"),
        &format!(
            r#"{{
...
fn environment_values_are_excluded_from_human_and_json_differences() {
    ...
    let home = tree.path().join("home");
    let project = tree.path().join("project");
    let claude_secret = "CLAUDE_SECRET_SENTINEL_2718";
    let codex_secret = "CODEX_SECRET_SENTINEL_3141";
    fs::create_dir_all(&home).expect("create home");
    write_empty_parity_config(&home);
    write_command_difference_with_secrets(&project, claude_secret, codex_secret);
    ...
    for output in [&human, &json] {
        ...
        assert!(
            !report.contains(claude_secret),
            "Claude environment value leaked\n{report}"
        );
        assert!(
            !report.contains(codex_secret),
